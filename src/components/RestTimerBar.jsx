...
export default function RestTimerBar({ timer }) {
      ...
      <div className={wrap}>
        <div className="mx-auto flex max-w-lg items-center gap-3 rounded-2xl bg-emerald-500 p-3 pl-4 text-zinc-950 shadow-2xl shadow-emerald-500/30">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-lg font-bold">{timer.kind === 'cardio' ? 'Time! Cardio block done' : 'Rest over — next set!'}</div>
            {timer.label && <div className="truncate text-sm opacity-80">{timer.label}</div>}
          </div>
          <button
            onClick={timer.skip}
            className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-zinc-950/15"
            aria-label="Dismiss"
          >
            <X size={22} />
          </button>
  ...
  const btn = 'grid h-12 w-11 shrink-0 place-items-center rounded-xl bg-zinc-800 text-sm font-semibold text-zinc-200 active:bg-zinc-700'
  ...
  return (
    <div className={wrap}>
      <div className="mx-auto max-w-lg overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-900/95 shadow-2xl backdrop-blur">
        ...
        >
          <div
            className={`h-full transition-[width] duration-200 ease-linear ${paused ? 'bg-zinc-500' : timer.kind === 'cardio' ? 'bg-emerald-400' : 'bg-cyan-400'}`}
        ...
        <div className="flex items-center gap-1.5 p-2.5">
          <div className="min-w-0 flex-1 pl-1">
            ...
            <div className="mt-1 truncate text-xs text-zinc-400">
              {paused ? 'Paused' : timer.label || (timer.kind === 'cardio' ? 'Cardio' : 'Resting')}
            </div>
          </div>
